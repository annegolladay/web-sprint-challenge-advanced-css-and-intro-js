...
function get20s(array) {
  ...
  for(let i = 0; i < array.length - 1; i++) {
    ...
    const dateArray = artist['years'].split('-');
    const birthDate = Number(dateArray[0]);
    const deathDate = Number(dateArray[1]);
    if(birthDate >= 1900 && deathDate <=2000) {
...
function lotsOfArt(array) {
  const artistsHundred = [];
  for(let i = 0; i < array.length; i++) {
    const artist = array[i];
    const numPaintings = Number(artist['paintings']);
    if (numPaintings > 100) {
      artistsHundred.push(artist['name']);
    }  
  }
  return artistsHundred;
}
...
const randomizeArray = [1, 2, 3, 4, 5];
...
console.log('Stretch 2:', randomize(randomizeArray));
